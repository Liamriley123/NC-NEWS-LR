...
describe('/api', () => {
  ...
  describe('/articles', () => {
    ...
    it.only('GET should accept queries such as ?sort_by and ?order to show us what we want in a specific order', () => request
      .get('/api/articles?sort_by=author&&order=desc')
      .expect(200)
      .then(({ body }) => {
        console.log(body.articles[1]);
        console.log(body.articles[9]);
        expect(body.articles).to.have.length(10);
        expect(body.articles).to.be.an('array');
        expect(body.articles[1]).to.haveOwnProperty('topic');
        expect(body.articles[1]).to.haveOwnProperty('title');
  ...
  describe('/articles/:article_id', () => {
    it('GET status:200 responds with an article object', () => request
      ...
      .then(({ body }) => {
        ...
        expect(body.article).to.haveOwnProperty('title');
        expect(body.article).to.haveOwnProperty('article_id');
        expect(body.article).to.haveOwnProperty('votes');
        expect(body.article).to.haveOwnProperty('created_at');
        expect(body.article).to.haveOwnProperty('topic');
